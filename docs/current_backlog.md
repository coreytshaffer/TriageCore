...
- CR-OC-001A: Mediated Single-File Effect Contract
  - Status: contract merged through PR #119 as `bfa1d6e`; bounded pure-module implementation complete on branch `cr-oc-001a-effect-contract-implementation`, open for review, not merged; runtime integration, persistence, IPC, capability issuance, and CR-OC-001B-E remain unauthorized. Cross-object binding is enforced so a linkage or capability bundle cannot mix an effect with an unrelated request, and the closed vocabulary widens from 11 to 16 codes so each names the condition that actually failed
