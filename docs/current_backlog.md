...
- CR-OC-001C: Constrained Single-File Replacement Executor
  - Status: requirements merged through PR #125 as `0b2b8a5`; implementation proposal merged as Part II (§25) through PR #126 as `0ab1522`; the §10.1a amendment merged through PR #127 as `8c0ad3c`; the §10.2a amendment merged through PR #129 as `8e19cdf`. **A third documentation-only amendment (§10.2b) merged through PR #130 as `5e41d6f`, triggered by three hosted Windows CI runs.** The implementation exists on draft PR #128 and **now conforms to §10.2b**: the supported-profile gate, present-and-protected healthy fixtures, the revised T20/T21/T35W evidence, M31, and a complete rebinding of all 31 mutants (21 Windows, 10 Ubuntu, zero survivors, zero invalid kills) are all in place. Local validation is 149 focused / 173 mandatory with a passing structured gate / 96 Ubuntu neutral plus 24 guards / 1664 full-repository. **Hosted validation has now passed**: run `30694001033` verified NTFS, ran 173 mandatory tests with zero skips, completed the structured gate, recorded the accepted `True->True` transition, and passed the supplemental symlink probe, with all four jobs green. GitHub Actions checked out the generated merge commit `c5e8a9e` (parents: base `5e41d6f`, head `257966b`), so the evidence binds to that tested tree and not to any later commit, and it is not a universal Windows guarantee. The implementation nonetheless remains **unmerged, not accepted, and not runtime-integrated**, and **merge authority has not been granted**. **The §10.2b amendment:** CR-OC-001C now supports only targets whose pre-mutation DACL is present, non-NULL, and protected from inheritance; absent, NULL, and inheritance-enabled DACLs are refused as `metadata_precondition_failed` with outcome `not_attempted`, before temporary-file creation and before any replacement attempt. This narrows the supported profile rather than weakening the invariant: the exact owner, DACL-state, control-bit, revision, ACE-count, ordered-ACE, and complete-ACE-byte comparison is unchanged, the `SE_DACL_AUTO_INHERITED` monotonic rule is unchanged, post-verification remains mandatory and authoritative, and unsupported targets are never normalized, repaired, protected automatically, or attempted. Hosted run `30689442321` showed a *successful* `ReplaceFileW` on an inheritance-enabled target differing in `dacl_auto_inherited`, `ace_count`, and `ace_bytes_or_order`; run `30690450931` produced a paired protected control that did not fail but was under-asserted, and is recorded deliberately because absence from a failure list is not evidence — as first written the control failed only on a metadata label beyond `dacl_auto_inherited`, so a refusal such as `temp_creation_failed` with no metadata differences would also have passed it, and the recorded result property was unrecoverable because the mandatory command failed before the gate or summary exposed it; run `30691068391` then passed a strengthened control requiring simultaneously a protected and nonempty fixture, `reason_code == ok`, outcome `replacement_verified`, target bytes equal to the proposed bytes, and metadata differences of none or `dacl_auto_inherited` alone, while the unprotected control reproduced the same three differences. What that establishes is narrow: on the hosted runner the unprotected profile violated the exact invariant while the present-and-protected profile completed a verified replacement under the same comparison. It does **not** prove inheritance recomputation is the underlying Windows mechanism — the diagnostic reported field labels only and established neither that every new or changed ACE was inherited nor that effective access was unchanged — and the environment (Windows Server 2025, build 10.0.26100, `windows-2025-vs2026`, Python 3.12.10, NTFS) is bounded evidence, not a universal Windows guarantee. An effective-permissions rule was rejected as materially harder to prove safely than exact equality, since ACE order affects access decisions and common effective-rights helpers omit owner rights, privileges, logon-session groups, resource-manager policy, and some inherited-deny cases. `pre_dacl_nonempty` was a diagnostic validity control proving the hosted success was not a trivial empty-ACL comparison, and is explicitly **not** a production requirement: a present, non-NULL, protected DACL with zero ACEs remains supported. §11's DACL carry-over language no longer implies that `ReplaceFileW` alone establishes general preservation. T21 is revised rather than supplemented, so obligations stay at 36 while mutants move from 30 to 31 (21 Windows, 10 Ubuntu); the new M31 defect is omitting the present-and-protected precondition or treating an inheritance-enabled target as supported, killed by T21[W]'s assertion that an unprotected target is refused before temporary-file creation or `ReplaceFileW`, with original bytes unchanged and no artifacts left behind.
  - Superseded status detail, retained for lineage — the §10.2a amendment: the pre-mutation ownership gate compared the target's owner against the token's `TokenUser`, but `TokenUser` identifies the token's user account while `TokenOwner` is the default owner SID Windows applies to newly created objects — and the latter is the quantity the gate actually needs, since its purpose is to establish that the temporary file this process creates will carry an owner compatible with the target, and `ReplaceFileW` documents preservation of the replaced file's DACL and several attributes but not its owner. Hosted run `30686689771` surfaced it: NTFS verification passed, 131 mandatory tests passed, 34 failed uniformly at `metadata_precondition_failed`, and the structured-result gate correctly did not run after the mandatory failure. Every failure reached the gate's closed refusal path before mutation; the job did not disclose SID values, so it does not establish the exact SID relationship on that runner, and local evidence had been green simply because the gate passed in the development environment — no persisted evidence established whether or why the two quantities coincided there. The amendment does not assert that hosted runners execute elevated or that any particular well-known SID was involved — those would be unverified explanations of why the values differed, and the contract defect does not depend on proving them. SID values are necessarily read into memory to compare owners at all; what is forbidden is disclosure and persistence, so no raw SID is logged, serialized, persisted, placed in JUnit, included in the bounded job summary, or otherwise emitted, and tests report only equality or inequality. T20 is revised rather than supplemented, so obligations stay at 36 while mutants move from 29 to 30 with a new M30 whose killer is a deterministic instrumented assertion that the adapter requests information class `TokenOwner`, deliberately not relying on the two SIDs differing on the machine under test — otherwise the mutant would survive on any host where they coincide, which is exactly how the defect escaped local evidence.

  - Superseded status detail, retained for lineage: a documentation-only amendment (§10.1a) corrected one requirement that implementation proved unimplementable as written. Implementation authority has been granted over the seven-path allowlist but the source files remain uncommitted pending this amendment. **The amendment:** the metadata invariant required strict equality of `SE_DACL_AUTO_INHERITED`, but a successful `ReplaceFileW` on real NTFS sets that bit on a file whose descriptor lacked it while preserving owner, DACL state, presence, protection, ACL revision, ACE count, and every complete ACE byte sequence byte-identically — so the contract as written would have reported `metadata_preservation_failed` for the first replacement of essentially any inherited-DACL file. That bit is now governed by a monotonic rule: `False -> False`, `False -> True`, and `True -> True` are accepted, `True -> False` is rejected, and the `False -> True` exception applies only when every other owner/DACL component is exactly preserved. Complete exclusion of the bit was rejected because Microsoft describes it as indicating the DACL supports the automatic-inheritance model rather than being a disposable provenance marker, and because excluding it would also accept `True -> False`, which no evidence supports. The evidence contract grows honestly rather than being held at its old totals: 34 obligations become 36 and 27 mutants become 29, adding acceptance and rejection tests for the rule plus mutants for strict equality and for ignoring the bit. The rule is proven deterministically off Windows on directly constructed snapshots, so both new mutants are killed on Ubuntu; the hosted Windows job observes and records whichever accepted transition the runner produces and fails only on `True -> False` or another metadata difference. Reproducing `False -> True` specifically is explicitly **not** a merge gate — `False -> False` is itself an accepted transition, so requiring the normalization would contradict the amendment and elevate one machine's observation into the platform guarantee the document refuses to make. The implementation proposal settles the seven-path allowlist (`triage_core/mediated_executor.py`, `triage_core/mediated_executor_win32.py`, `tests/test_mediated_executor.py`, `.github/workflows/tests.yml`, and the three documentation paths), resolving the requirements-era unresolved candidates by explicit operator decision rather than silently: a dedicated repeatable `windows_executor` job on `windows-latest`/Python 3.12 is the evidence mechanism, added additively so the existing Ubuntu 3.10/3.11/3.12 matrix stays intact, with local Windows runs permitted only as supplement and never as substitute. The module split puts policy, the trusted registry, the closed vocabulary, pure `ReplaceFileW` classification, the pure snapshot comparison, the process-local lock, the Windows gate, and sequencing in the neutral core, and confines every Win32 structure walk to a private Windows-only adapter that decides nothing; the core imports the adapter dynamically and only after the Windows gate, so it stays import-safe and filesystem-silent on Ubuntu. The dependency is genuinely one-way and no type crosses it upward: the adapter owns a private `Win32SecurityCapture` of mechanism primitives and imports no TriageCore module at all, while the core validates that capture and converts it into the core-owned `SecuritySnapshot`, where the three-valued DACL classification and every comparison decision live — so platform-neutral tests construct `SecuritySnapshot` directly, and no third shared module is introduced. That split is what lets seven mutants — including all five ACE-comparison mutants — be killed on Ubuntu rather than only on Windows. All 34 required obligations are assigned a venue (20 Windows-only, 6 Ubuntu-only, 8 both) and all 27 mutants a designated killer. Mandatory Windows evidence cannot skip: the job writes JUnit XML under `RUNNER_TEMP`, never uploads it, and fails when the mandatory group reports any skip or the report is missing or unparseable, since a broken platform gate must turn CI red rather than silently green. T6 and T7 are satisfied by junctions, which the hosted runner reliably supports without elevation, while the symbolic-link variant is a supplemental non-gating probe held outside the 34 obligations and outside the zero-skip calculation, reporting pass or environment-unavailable and never counting toward acceptance. The Windows install command is pinned to exactly `.[dev]` with no conditional fallback — failure to collect is a stop condition rather than a silent broadening — the job runs with `permissions: contents: read` and needs no secrets, write scope, or uploaded diagnostics, and the existing Ubuntu job keeps its current install unchanged. The closed set of injection seams is enumerated and labeled so seam-injected branches are never counted as genuine-filesystem evidence. Sixteen stop conditions require renewed approval, and the proposal states in its own text that neither the requirements merge nor the proposal itself is implementation authority: work may begin only after the proposal is reviewed, its allowlist accepted, the CI evidence design accepted, the module boundary accepted, and explicit human implementation approval granted.
  - Purpose: Define the requirements for the one constrained filesystem effect the mediated lane still lacks — given an already constructed CR-OC-001A effect, independently supplied exact proposed bytes, and a trusted `target_file_id` -> target mapping, perform at most one single-file content replacement and report the outcome honestly. The revised proposal is a Windows/NTFS executor over a platform-neutral pure core: an earlier two-profile draft was rejected in review because the POSIX and Windows mutating paths shared almost nothing safety-bearing — different containment mechanism, primitive, failure contract, metadata truth, and identity semantics — which was two executors under one name, with the POSIX half serving mainly to let Ubuntu CI stand in for evidence about the platform the lane actually targets. Non-Windows platforms fail closed as `platform_unsupported`; Ubuntu CI exercises only the pure core and the fail-closed gate, and acceptance requires real Windows/NTFS evidence from either a separately approved Windows CI job (`.github/workflows/tests.yml` is a named allowlist candidate, not pre-approved) or a recorded local Windows run. Windows claims no namespace atomicity — Microsoft documents none — and uses `ReplaceFileW` via stdlib `ctypes` for its documented DACL-preservation list and per-error-code state contract, with a private random same-directory backup name always supplied because the documented no-backup failure mode deletes the target; the backup's full lifecycle is specified, including retention on every non-verified outcome, deletion only after complete post-verification, no restart-time scanning or cleanup, and human-only recovery gated on verifying the backup against `expected_pre_digest`. Resolution is exclusively through `target_file_id` against an operator-constructed frozen registry; `canonical_relpath` is compared byte-for-byte as evidence and never joins, resolves, or opens anything; containment is reparse-attribute rejection plus final-path verification of the actually opened handle, never a lexical prefix check. The owner/DACL invariant is a structured comparison — semantic owner-SID equality, three-state DACL classification (absent, NULL, present including present-empty), pinned control bits, ACL revision and ACE count, and ordered complete per-ACE byte-sequence equality with fail-closed `AceSize` bounds validation, so object-ACE GUIDs, callback application data, and inheritance fields all participate while unknown ACE types are compared as opaque wholes and ACL slack space is excluded — never raw SDDL string equality, which is not documented as canonical; SACL and primary group are excluded with stated reasons. All three documented `ReplaceFileW` special errors are mapped by name: `ERROR_UNABLE_TO_REMOVE_REPLACED` and `ERROR_UNABLE_TO_MOVE_REPLACEMENT` to `target_unchanged` on their documented name-retention statements, `ERROR_UNABLE_TO_MOVE_REPLACEMENT_2` and every undocumented failure to the ambiguous outcome. The identity claim is stated honestly: a validation handle proves things about the original object only, `ReplaceFileW` resolves the target name independently of any held handle, an identity re-probe narrows but cannot close the race, the no-concurrent-writer assumption is load-bearing, and post-verification re-resolves entirely through the frozen registry because the post-replacement object legitimately has a new identity. Observing the postcondition is explicitly not proof that this invocation performed the replacement. Degenerate `pre == post` effects are outside the operation contract — this executor performs an actual content transition, so their rejection is a scope and API rule, explicitly not replay prevention, which is CR-OC-001B's authority alone; a future separately scoped observational operation could define `already_at_authorized_postcondition`, and this executor implements no such operation. No automatic rollback, ever — restoring pre-content, including from the backup, would be a second mutation without a fresh guard. Thirty-four test-contract items tagged neutral-vs-Windows and twenty-seven mutants are named before any code exists, with the one unkillable equality-preserving resolution mutant recorded honestly as reviewer-controlled. The executor is invoked only by tests; it issues no capability, touches no reservation, writes no ledger, and no runtime module may import it. Implementation, if later approved, is expected to stay within `triage_core/mediated_executor.py`, its test file, and the three documentation paths.
...
- CR-DD-013: Observed Local Capability Binding for Governed Routing
  ...
  - Purpose: Bind the existing validated `local_backend_probe_record.v1` observation into `ResilienceRouteInput` so route selection stops asserting local availability it has never checked. Distinguishes observed-available, observed-unavailable, configured, and unknown; forbids the `tc run` construction path from synthesizing `True` from unknown evidence; pins `probe_disabled`, missing, stale, and validation-failed observations to unknown rather than available; treats an observed failure as conclusive for dependent routes while a single reachability result may not assert both `local_fast` and `local_heavy`; keeps absent memory/model evidence unknown rather than zero; and preserves the existing local-only fail-closed path instead of adding a second one. Direct callers supplying explicit booleans retain current behavior. Proposes exactly one additive structured field and no new ledger schema. No new probe, routing heuristic, provider, circuit breaker, or CR-YK work. Independent of the CR-DD-012 lane and carries no CR-YK-002 gate.
...
- **[proposed; implementation unauthorized] Observed local capability binding (CR-DD-013)**: Consumes the merged CR-114/CR-118/CR-119 probe record to populate `ResilienceRouteInput`, replacing hardcoded optimistic local availability with an observation that distinguishes observed-available, observed-unavailable, configured, and unknown. Unknown is never promoted to availability and the construction path may not synthesize `True` from it, so operators with neither an observation nor an explicit configured declaration will see local capability treated as unknown rather than healthy; direct callers supplying explicit booleans are unaffected. Making unknown a hard denial, retry, re-probe, circuit breakers, per-route backend separation, and execution-time verification remain later separately approved work. This is the first M1 slice and is independent of CR-DD-012B and the CR-YK lane.
