...
- CR-OC-001A: Mediated Single-File Effect Contract
  ...
  - Purpose: Define how one exact single-file content transition is represented, validated, and bound, as the first slice of the mediated OpenClaw experiment. A pure module would describe a replacement by allowlisted `target_file_id` rather than caller-supplied path, pin it to exact pre- and post-content digests, keep client-declared invocation context structurally separate from a broker-generated connection identifier, define request-replay semantics keyed to the client request rather than the capability, and expose a metadata-only persistent projection that never carries proposed bytes. It maps onto the merged CR-YK-002 capability without changing `AuthorizationRequest` or the capability schema. The claim it may support is bounded: an exact authorized pre-content digest became an exact authorized post-content digest — not exact filesystem state, not an authenticated context, not replay prevention, not path safety, and not that any file changed. No file access, IPC, persistence, capability issuance, named pipes, OpenClaw configuration, or mutation. CR-OC-001B through CR-OC-001E each require separate approval.
...
For the mediated OpenClaw experiment, CR-OC-001A proposes the pure single-file effect contract as the first of five slices: CR-OC-001A the effect contract, CR-OC-001B atomic client-request reservation, CR-OC-001C the constrained replacement executor, CR-OC-001D the privilege-separated broker and hardened named pipe, CR-OC-001E the exclusive OpenClaw tool and effective-schema evaluation. CR-OC-001A is documentation-only and unauthorized for implementation, and each later slice requires its own approval. The named-pipe hardening requirements — `PIPE_REJECT_REMOTE_CLIENTS`, an explicit security descriptor denying `NT AUTHORITY\NETWORK`, a random per-run pipe name with `FILE_FLAG_FIRST_PIPE_INSTANCE`, and shim-side verification of the pipe server before any content is transmitted — are recorded as CR-OC-001D requirements and are not satisfied by any earlier slice. Do not treat a defined contract as an enforced one: CR-OC-001A defines replay semantics without preventing replay, and names a broker connection identifier without authenticating it.
