...
- Checked the lettered lineages for fit before defaulting to plain numeric:
  ...
  - None of the five lettered lineages govern repository architecture-mapping
    documentation. The prior comparable work — the architecture-mapping commit
    `6f5970d` ("docs(architecture): map current TriageCore system") that created
    `current_system_architecture.md` itself, and CR-131 immediately preceding this one —
    both sit in the plain-numeric lineage.
...
2. Explicitly account for **both** packaged entry points defined in `pyproject.toml`'s
   ...
   (`triage_core.cli:main`) — rather than describing only the former.
...
4. Add an honest Worker Council entry stating that the path is **present and wired, but
   currently non-operational for its configured role set**: the TriageDesk GUI's
   dispatch call requests exactly the roles `repo_mapper`, `code_repair`, and
   `validator`; orchestration resolves each work order's worker by an exact registry-key
   lookup and records a `"Worker role ... not found"` failure when that lookup misses;
   and the registry exposes only the keys `context_planner`, `test_stubber`,
   `review_worker`, and `implementer`. The lookup miss occurs before any worker or
   backend execution for those three configured UI roles — this is a directly observed,
   causal reachability gap, not a speculative one. The existence of similarly named
   skill Markdown files (`repo_mapper.md`, `code_repair.md`, `validator.md`) does not
   repair that gap, since nothing in the current call path ever constructs a worker
   under those role names to read them.
5. Update the authoritative-source list (the document's "Authoritative Sources
   ...
   `triage_core/orchestration.py`, `triage_core/worker_registry.py`, the relevant
   TriageDesk UI call site (`triage_core/ui/app.py`), and focused evidence/tests
   (`tests/test_cli.py`, `tests/test_ui_smoke.py`) — mirroring the granularity the
   document already uses for the `tc`-path modules it lists.
...
2. **A currently non-operational subsystem presented, by omission, as if it does not
   ...
   `triage_core/orchestration.py`'s `ProjectManager.dispatch_task`. That function
   resolves each queued work order's worker via `self.registry.get_worker(order.assigned_role)`
   — an exact-key lookup against `triage_core/worker_registry.py`'s `WorkerRegistry`,
   which registers only the keys `context_planner`, `test_stubber`, `review_worker`, and
   `implementer`. When the lookup misses, the order is marked failed with
   `f"Worker role {order.assigned_role} not found"` before any worker or backend
   execution occurs. This means the failure for all three of the GUI's configured
   roles — `repo_mapper`, `code_repair`, `validator` — is causal and observed directly
   in the call path, not inferred: dispatch always requests those three exact names, the
   registry never exposes them, and the lookup always misses before execution. Skill
   prompt files matching those role names exist on disk
   (`triage_core/skills/repo_mapper.md`, `code_repair.md`, `validator.md`) and are
   correctly authored for their intended roles, but nothing in this call path ever
...
- [ ] Proposed eventual documentation scope and explicit exclusions are stated clearly
      enough that a later design-review stage has unambiguous boundaries, and that the
      runtime defect is not smuggled into a documentation-only implementation.
...
3. the CR namespace census is reproducible (`CR-132` still does not collide) at the
   time of review;
4. the two `[project.scripts]` entries, the `ui/app.py` dispatch call's exact
   `required_roles` list, the `WorkerRegistry` key set, and the existence of the three
   named skill files are still as described above at the time of review, since this
   proposal's problem statement depends on those exact observations.
